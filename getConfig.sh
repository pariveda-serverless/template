#!/bin/bash
aws cloudformation list-exports --query 'Exports[?Name==`brett-security-'"$STAGE"'`].Value' --output text > lambdarolearn.txt
aws cloudformation list-exports --query 'Exports[?Name==`brett-security-secrets-'"$STAGE"'`].Value' --output text > s3.txt
